...
for files in file_list:
    ...
    with open(".//result//" + files) as file1:
        ...
        scc_lsit = [float(line.split("\t")[1]) for line in f1[2:]]

        
        prot_drug = files.replace("predicted_", "").replace("_HUMAN_drugs_w_all_model_scc.txt", "")
        ...
        ax.text(xend*0.43, yend*0.87, "Number of Drugs = " + drug_number, fontsize=25)

        fig.tight_layout()
        fig.savefig(".//distribution_fig//SCC_of_own_predicted_" + prot_drug + "_drug_predicted_other_models.png")
        plt.clf()
        counter += 1
